...
export class TerrainGeometry extends BufferGeometry {
  ...
  constructor(readonly data: QuantizedMeshData, rectangle: Rectangle) {
    super();
    ...
    this.position = new Vector3(header.boundingSphereCenterX, header.boundingSphereCenterY, header.boundingSphereCenterZ);
    ...
    if (extensions?.vertexNormals != null) {
      const normal = this.createPackedEncodedNormalAttribute(extensions.vertexNormals);
  ...
  override computeVertexNormals(): void {
    ...
    for (let index = 0, normalIndex = 0; index < array.length; index += 3, normalIndex += 2) {
  ...
  createPositionAttribute(
    ...
    for (let index = 0, vertexIndex = 0; index < array.length; index += 3, ++vertexIndex) {
  ...
  createUvAttribute(vertexData: Uint16Array): BufferAttribute {
    ...
    for (let index = 0, vertexIndex = 0; index < array.length; index += 2, ++vertexIndex) {
  ...
  createPackedEncodedNormalAttribute(vertexNormals: Uint8Array): BufferAttribute {
    invariant(vertexNormals.length % 2 === 0);
    const array = new Float32Array(vertexNormals.length / 2);
    for (let index = 0, normalIndex = 0; index < array.length; ++index, normalIndex += 2) {
      const x = vertexNormals[normalIndex];
      const y = vertexNormals[normalIndex + 1];
      array[index] = 256 * x + y;
    }
